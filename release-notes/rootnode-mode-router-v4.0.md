...
**Surface:** Claude Code • **Tier:** Model-compatible (T1)

### Install

Download `rootnode-mode-router-cc.zip` below, extract it into `~/.claude/skills/` (user-level — available across every repo) or `.claude/skills/` (per-repo). The extracted folder name matches the Skill name.
