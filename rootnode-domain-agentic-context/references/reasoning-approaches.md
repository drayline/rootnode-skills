...
4. Design detection mechanisms: How does the agent know it has failed? Explicit error responses from tools are easy to detect. Silent failures — wrong answers that look plausible, gradual context corruption from accumulated stale data, hallucinated actions that happen to succeed — are hard. For high-stakes decisions, design external-artifact verification steps: cross-reference the agent's output against a second source, re-derive the result via an independent tool, or confirm with the user before acting. Do not rely on the agent "checking its own work" as the verification mechanism — Opus 5 already does this automatically, and instructing self-directed re-checking compounds into over-verification (see `root_OPTIMIZATION_REFERENCE.md` tendency #11). The verification that catches silent failures is the one that grounds against something outside the agent's own output.
